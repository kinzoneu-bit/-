          ...
          {PHASE_ORDER.map(fromP => {
            ...
            return (
              <div key={fromP} style={{ display: "grid", gridTemplateColumns: `1.2fr repeat(${PHASE_ORDER.length + 1},1fr)`, borderTop: `1px solid ${C.line}`, fontSize: 12, background: C.panel }}>
                ...
                {PHASE_ORDER.map(toP => (
                  <div key={toP} style={{ padding: "9px 12px", textAlign: "center", color: row.dist[toP] ? "#0f5e9c" : C.sub, fontWeight: row.dist[toP] ? 700 : 500 }}>
                    {row.dist[toP] || "—"}
                  </div>
                ))}
                <div style={{ padding: "9px 12px", textAlign: "center", color: "#0f5e9c", fontWeight: 700, fontSize: 13 }}>{row.total || "—"}</div>
              </div>
            );
          })}
        </div>
